...
from ..resizer import bilinear_interpolation
...
class DRDetector(DenseDetector):
    ...
    def forward(self, batched_inputs: List[Dict[str, Tensor]]):
        ...
        if self.resizer is not None:
            ...
            for i, x in enumerate(batched_inputs):
                ...
                resized_img = bilinear_interpolation(x["image"], 
                                                     torch.round(orn_shape * cur_sf)).squeeze(0)
        ...
        features = self.backbone(images.tensor)
        features = [features[f] for f in self.head_in_features]
        if any([x.isnan().any() for x in features]):    print(out_dict)
        assert not any([x.isnan().any() for x in features])
        predictions = self.head(features)
        
        if self.training:
            assert not torch.jit.is_scripting(), "Not supported"
            assert "instances" in resized_inputs[0], "Instance annotations are missing in training!"
            gt_instances = [x["instances"].to(self.device) for x in resized_inputs]
            return self.forward_training(images, features, predictions, gt_instances, out_dict)
        else:
            results = self.forward_inference(images, features, predictions)
            if torch.jit.is_scripting():
                return results
            ...
            processed_results = []
            for results_per_image, input_per_image, image_size in zip(
                results, resized_inputs, images.image_sizes
            ):
                height = input_per_image.get("height", image_size[0])
                width = input_per_image.get("width", image_size[1])
                r = detector_postprocess(results_per_image, height, width)
