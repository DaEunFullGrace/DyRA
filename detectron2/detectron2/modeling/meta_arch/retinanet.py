...
class RetinaNet(DRDetector):
    ...
    def losses(self, anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes, sf_dicts=None):
        ...
        if self.resizer is not None:
            balance_dict = self.resizer.balance_loss(anchors, self.box2box_transform,
                                                     gt_boxes, pos_mask, loss_box_reg)
            balance_dict = dict({k:balance_dict[k]*loc_val for k in balance_dict})
        else:
            balance_dict = {}
            
        return dict({"loss_cls": loss_cls / normalizer, 
                     "loss_box_reg": loss_box_reg.sum() / normalizer,
                     "loss_ps": sf_dicts["loss_ps"]*loc_val}, **balance_dict)
