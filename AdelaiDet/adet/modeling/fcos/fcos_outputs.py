...
class FCOSOutputs(nn.Module):
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, 
               locations, gt_instances, top_feats=None, loc_reduction="sum"):
        """
        ...
        return self.fcos_losses(instances, loc_reduction=loc_reduction)

    def fcos_losses(self, instances, loc_reduction="sum"):
        losses, extras = {}, {}
        ...
        if self.box_quality == "ctrness":
            ...
            reg_loss = self.loc_loss_func(ious, gious, ctrness_targets, 
                                          reduction=loc_reduction) / loss_denorm
            ...
            reg_loss = self.loc_loss_func(ious, gious, reduction=loc_reduction) / num_pos_avg
            losses["loss_fcos_loc"] = reg_loss
    ...
    def predict_proposals(
            ...
            locations, image_sizes, top_feats=None, thresh=None,
    ):
        if self.training:
    ...
    def forward_for_single_feature_map(
        ...
        if self.thresh_with_ctr:    logits_pred = logits_pred * ctrness_pred[:, :, None]

        candidate_inds = logits_pred > self.pre_nms_thresh  # if (thresh is None) else thresh)
        pre_nms_top_n = candidate_inds.reshape(N, -1).sum(1)
        
        if self.pre_nms_topk is not None:
