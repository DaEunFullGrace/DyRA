...
class FCOS(nn.Module):
    ...
    def forward(self, images, features, gt_instances=None, top_module=None, reduction="sum"):
        """
        Arguments:
        ...
        if self.training:
            results, losses = self.fcos_outputs.losses(
                ...
                locations, gt_instances, top_feats, loc_reduction=reduction,
            )
            ...
            if self.yield_proposal:
                with torch.no_grad():
                    results["proposals"] = self.fcos_outputs.predict_proposals(
                        logits_pred, reg_pred, ctrness_pred,
                        locations, images.image_sizes, top_feats, # thresh=0,
                    )
