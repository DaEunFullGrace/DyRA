...
__global__ void ml_nms_kernel(const int n_boxes, const float nms_overlap_thresh,
  ...
  if (threadIdx.x < row_size) {
    ...
    const int col_blocks = at::ceil_div(n_boxes, threadsPerBlock); //THCCeilDiv
...
at::Tensor ml_nms_cuda(const at::Tensor boxes, const float nms_overlap_thresh) {
  ...
  const int col_blocks = at::ceil_div(boxes_num, threadsPerBlock); //THCCeilDiv
  ...
  mask_dev = (unsigned long long*) c10::cuda::CUDACachingAllocator::raw_alloc(boxes_num * col_blocks * sizeof(unsigned long long)); //THCudaMalloc

  dim3 blocks(at::ceil_div(boxes_num, threadsPerBlock),
              at::ceil_div(boxes_num, threadsPerBlock)); //THCCeilDiv
  ...
  c10::cuda::CUDACachingAllocator::raw_alloc(cudaMemcpy(&mask_host[0],
                        ...
                        cudaMemcpyDeviceToHost)); //THCudaCheck
  ...
  c10::cuda::CUDACachingAllocator::raw_delete(mask_dev); //THCudaFree
